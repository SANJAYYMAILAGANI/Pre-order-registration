        ...
        <div class="row">
            ...
            <div class="col-sm-4 text-center" style="border:2px solid gray;padding:50px;">
                ...
                function test_input($data) {
                    ...
                    return htmlspecialchars($data); // Safe against XSS
                }
                ...
                // Verify Old Password
                $stmt = $conn->prepare("SELECT admin_password FROM db_evoting.tbl_admin WHERE admin_username='admin'");
                $stmt->execute();
                $result = $stmt->get_result();
                $row = $result->fetch_assoc();

                if ($row && $row['admin_password'] === $old) {
                    // Password is valid, update to new password
                    $update_stmt = $conn->prepare("UPDATE db_evoting.tbl_admin SET admin_password=? WHERE admin_username='admin'");
                    $update_stmt->bind_param("s", $new);
